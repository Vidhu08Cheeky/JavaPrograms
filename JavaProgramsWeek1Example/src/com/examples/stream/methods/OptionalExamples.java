package com.examples.stream.methods;
...
public class OptionalExamples {
	...
	public static void main(String[] args) {
        ...
        List<Integer> ids= list.stream()
                .filter(e->e.getEmpName().startsWith("M"))
                .map(e->e.getEmpId())
                .collect(Collectors.toList());
        ...
        List<Integer> ids2= list.stream()
                .filter(e->e.getEmpName().startsWith("M")) //employees names starting 'M'
                .map(e->e.getEmpId()*10) //collecting only IDs * 10 //give to peek()
        ...
        empByName = list.stream().collect(Collectors.groupingBy(Employee::getEmpName));
